...
static void NPC_StuShop_selectWindow( int meindex, int toindex, int num,int select);
BOOL StuShop_GetMenuStr( int meindex, int toindex, char *npcarg, char *token, int index );
int StuShop_ShowMenulist( char *npcarg );
...
void NPC_StushopTalked( int meindex, int talkerindex, char *msg, int color )
{
	...
	NPC_StuShop_selectWindow( meindex, talkerindex, WINDOW_START, 0);
}

void NPC_StushopWindowTalked( int meindex, int talkerindex, int seqno,
								int select, char *data)
{
	...
	case NPC_TRANSERMAN_SELECT:
		NPC_StuShop_selectWindow( meindex, talkerindex, WINDOW_SELECT, atoi( data));
		break;
	case NPC_TRANSERMAN_WARP:
		...
		{
				NPC_StuShop_selectWindow( meindex, talkerindex, WINDOW_NEXT, atoi( data));
				return;
		}
		if( select == WINDOW_BUTTONTYPE_PREV )
		{
				NPC_StuShop_selectWindow( meindex, talkerindex, WINDOW_PREV, atoi( data));
				return;
		}
		NPC_StuShop_selectWindow( meindex, talkerindex, WINDOW_WARP, atoi( data));
		break;
	case NPC_TRANSERMAN_END:
		NPC_StuShop_selectWindow( meindex, talkerindex, WINDOW_END, atoi( data));
		break;
	}
...
static void NPC_StuShop_selectWindow( int meindex, int toindex, int num,int select)
{
	...
	if(NPC_Util_GetArgStr( meindex, npcarg, sizeof(npcarg))==NULL){
		print("StuShop_MAN: GetArgStrErr!!");
		return;
	}
	...
	case WINDOW_START:
		if( NPC_Util_GetStrFromStrWithDelim( npcarg, "START_MSG", token, sizeof( token) ) == NULL) {
			print("StuShop Get START_MSG ERROR !");
			return;
		}
	...
	case WINDOW_SELECT:
		...
		page=StuShop_ShowMenulist(npcarg);
		if( StuShop_GetMenuStr( meindex, toindex, npcarg, token,CHAR_getWorkInt( toindex, CHAR_WORKLISTPAGE)) == FALSE )	{
			print( "StuShop GetMenu ERROR !!");
			return;
		}
...
BOOL StuShop_GetMenuStr( int meindex, int toindex, char *npcarg, char *token,int index )
{
	...
	sprintf( token, "你当前的徒弟数是:%d        %d/%d页\n",CHAR_getInt( toindex , CHAR_STUNUM), index+1, page);
