...
//        tile[i] = ntohs( tile[i] );
    for( i = 0 ; i < xsiz * ysiz ; i ++ ){
        ...
        if( !IsValidImagenumber( tile[i] ) ){
            fprint("1地图的图片有问题:%d x:%d y:%d 数量:%d\n",
...
{
	int i=0;
	int WaterWord[]={8101,8015,8029,8028,8027,817,8007,8100,151,160,161,-1};
	while( WaterWord[i] != -1 ){
		if( floor == WaterWord[i] ){
			return 1;
		}
		i++;
	}
...
BOOL MAP_SetExWarp(int mapid , int fl, int x, int y, int type)
{
	...
	MAP_map[tomapindex].MapType = type;//地图类型？ 目前不清楚干嘛的 保留修改的权利
	return TRUE;
}
  ...
  if ((tomapindex = MAP_getfloorIndex(mapid)) == -1)
   ...
   int mapstartid = getCopymapstartingID();//获得副本地图起始ID
    ...
    for(i=0;i<MAP_mapnum_index;i++)//遍历所有地图号ID
    	{
    		if (MAP_map[i].id == j) break;//如果地图ID以被使用
    	}
   ...
   if (mymapid == -1) {
    ...
    for(i=0;i<MAP_mapnum_index;i++)//先找一个空间，是否有被释放，如果有，责不需要重新开辟空间了。
    	{
...
//    	print( "重新分配地图内存成功！\n");
	memcpy( MAP_map , MAP_map2 ,sizeof(MAP_Map) * (MAP_mapnum-100));//保存之前的数据
    freeMemory( MAP_map2 );
    MAP_map2 = 0;
    }
    short   *tile=0;
    ...
    tile = allocateMemory( sizeof( short ) * MAP_map[tomapindex].xsiz * MAP_map[tomapindex].ysiz );//分配地图内存
    if( tile == NULL ){
        print( "无法分配内存给地图图层\n");
        return -1;
    }

    obj = allocateMemory( sizeof( short ) * MAP_map[tomapindex].xsiz * MAP_map[tomapindex].ysiz );//分配地图对象内存
    if( obj == NULL ){
         print( "无法分配内存给地图对象\n");
        freeMemory( tile );
        return -1;
    ...
    olink = allocateMemory( sizeof(MAP_Objlink*) * MAP_map[tomapindex].xsiz * MAP_map[tomapindex].ysiz );//分配地图链接内存
    if( olink == 0 ){
        print( "无法分配内存给地图链接\n");
        freeMemory( obj );
        return -1;
    }
       memcpy( &tile , &MAP_map[tomapindex].tile ,sizeof(MAP_map[tomapindex].tile));
       memcpy( &obj , &MAP_map[tomapindex].obj ,sizeof(MAP_map[tomapindex].obj));
        for( i=0 ; i< xy ; i ++ ){
        olink[i] = 0;
        }
    MAP_map[makemapindex].id = mymapid;
    ...
    MAP_map[makemapindex].olink = olink;
		MAP_map[makemapindex].startpoint = (2006<<16)+(20 << 8)+(15<<0);//默认为传送回渔村村长家
					MAP_map[makemapindex].MapType = 0;

    MAP_idjumptbl[mymapid] = makemapindex;
    if ( makemapindex >= MAP_mapnum_index) MAP_mapnum_index = makemapindex+1;
fbmap[mymapid-mapstartid] = mapid;
return mymapid;
}
...
BOOL MAP_DelMap(int mapid )
{
	...
	if (MAP_map[tomapindex].tile != 0) freeMemory( MAP_map[tomapindex].tile );//释放掉旧资料
	if (MAP_map[tomapindex].obj != 0) freeMemory( MAP_map[tomapindex].obj );//释放掉旧资料
	freeMemory( MAP_map[tomapindex].olink );//释放掉旧资料
	MAP_map[tomapindex].startpoint = -1;//释放退出地图传送点
	MAP_map[tomapindex].MapType = 0;//地图类型？ 目前不清楚干嘛的
	MAP_idjumptbl[mapid] = -1;//地图索引
	return TRUE;
}
#endif
